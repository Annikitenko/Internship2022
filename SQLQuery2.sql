...
SELECT sector_id, sector_name FROM sectors
WHERE sector_name IN ('value 1', 'value 3', 'value 4', 'value 5') AND sector_name NOT IN ('value 2','value 7');
...
SELECT COUNT(Customer_Id) AS 'The number of records in Customers' FROM customers;
...
SELECT COUNT(pack_id) AS quantity_of_packages, AVG(monthly_discount) AS average_monthly_discount FROM customers
GROUP BY monthly_discount
ORDER BY average_monthly_discount;
...
SELECT city, ROUND(AVG(monthly_discount),2) as average_monthly_discount from customers
GROUP BY city;
